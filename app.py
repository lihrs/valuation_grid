...
# ============================================================
# v5.24: 7天短线策略 API（独立实现，不依赖低频网格）
# ============================================================

# 导入独立短线模块
from grid.shortterm import (
    get_config as st_get_config,
    update_config as st_update_config,
    reset_config as st_reset_config,
    get_all_positions as st_get_all_positions,
    get_position as st_get_position,
    add_position as st_add_position,
    sell_position as st_sell_position,
    remove_position as st_remove_position,
    update_position_nav as st_update_nav,
    generate_signal as st_generate_signal,
    generate_all_signals as st_generate_all_signals,
    get_history as st_get_history,
)


class ShortTermConfigRequest(BaseModel):
    """短线策略配置请求"""
    stop_loss_pct: Optional[float] = None
    stop_loss_max_pct: Optional[float] = None
    take_profit_target: Optional[float] = None
    take_profit_tier1: Optional[float] = None
    take_profit_tier2: Optional[float] = None
    trail_stop_pct: Optional[float] = None
    trail_activate_profit: Optional[float] = None
    first_build_ratio: Optional[float] = None
    supplement_ratio: Optional[float] = None
    max_supplement_count: Optional[int] = None
    cooldown_days: Optional[int] = None
    dip_buy_threshold: Optional[float] = None
    bounce_buy_threshold: Optional[float] = None
...
class ShortTermSellRequest(BaseModel):
    """短线卖出请求"""
    batch_id: str
    sell_shares: float
    sell_nav: Optional[float] = None
...
class ShortTermNavRequest(BaseModel):
    """补录净值请求"""
    batch_id: str
    nav: float
...
@app.get("/v1/shortterm/positions")
def st_get_positions_api():
    """获取所有短线持仓"""
    return st_get_all_positions()


@app.get("/v1/shortterm/position/{fund_code}")
def st_get_position_api(fund_code: str):
    """获取单只基金短线持仓"""
    return st_get_position(fund_code)


@app.post("/v1/shortterm/position/{fund_code}/buy")
def st_buy_api(fund_code: str, req: ShortTermBuyRequest):
    """短线买入"""
    if req.amount <= 0:
        raise HTTPException(status_code=400, detail="金额必须大于0")
    try:
        batch = st_add_position(
            fund_code, req.amount, req.nav, req.note or "",
            req.buy_date, req.max_position, req.fund_name or ""
        )
        return {"success": True, "batch": batch}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@app.post("/v1/shortterm/position/{fund_code}/sell")
def st_sell_api(fund_code: str, req: ShortTermSellRequest):
    """短线卖出"""
    if req.sell_shares <= 0:
        raise HTTPException(status_code=400, detail="卖出份额必须大于0")
    try:
        result = st_sell_position(
            fund_code, req.batch_id, req.sell_shares,
            req.sell_nav, req.sell_date
        )
        return {"success": True, **result}
    except ValueError as e:
...
@app.delete("/v1/shortterm/position/{fund_code}")
def st_remove_api(fund_code: str):
    """移除短线持仓"""
    if st_remove_position(fund_code):
        return {"success": True, "message": f"已移除 {fund_code}"}
    raise HTTPException(status_code=404, detail=f"基金 {fund_code} 不存在")


@app.put("/v1/shortterm/position/{fund_code}/nav")
def st_update_nav_api(fund_code: str, req: ShortTermNavRequest):
    """补录买入净值"""
    if req.nav <= 0:
        raise HTTPException(status_code=400, detail="净值必须大于0")
    try:
        batch = st_update_nav(fund_code, req.batch_id, req.nav)
        return {"success": True, "batch": batch}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
@app.get("/v1/shortterm/signals")
def st_get_signals_api():
    """获取所有短线信号"""
    return st_generate_all_signals()


@app.get("/v1/shortterm/signal/{fund_code}")
def st_get_signal_api(fund_code: str):
    """获取单只基金短线信号"""
    return st_generate_signal(fund_code)


# ===== 配置 API =====

@app.get("/v1/shortterm/config")
def st_get_config_api():
    """获取短线配置"""
    return st_get_config()


@app.post("/v1/shortterm/config")
def st_update_config_api(req: ShortTermConfigRequest):
    """更新短线配置"""
    updates = {k: v for k, v in req.model_dump().items() if v is not None}
    if not updates:
        return {"success": True, "message": "无更新", "config": st_get_config()}
    new_config = st_update_config(updates)
    return {"success": True, "message": "配置已更新", "config": new_config}


@app.post("/v1/shortterm/config/reset")
def st_reset_config_api():
    """重置短线配置"""
    config = st_reset_config()
    return {"success": True, "message": "已重置为默认配置", "config": config}


# ===== 历史记录 API =====

@app.get("/v1/shortterm/history")
def st_get_history_api(limit: int = 30):
    """获取短线信号历史"""
    return st_get_history(limit=limit)


@app.get("/v1/shortterm/history/{fund_code}")
def st_get_fund_history_api(fund_code: str, limit: int = 30):
    """获取单只基金短线信号历史"""
    return st_get_history(fund_code=fund_code, limit=limit)


# ===== 板块基金选择 API =====

@app.get("/v1/shortterm/sector-funds")
def st_get_sector_funds_api():
    """获取板块中的基金列表（用于选择添加）"""
    state = load_state()
    result = []
    seen = set()
