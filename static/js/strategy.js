...
async function submitBuy() {
    ...
    if (amount === 0) {
        try {
            ...
            const res = await fetch(`${API_BASE}/v1/position/${encodeURIComponent(code)}/watch`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function submitSell() {
    ...
    try {
        ...
        if (mode === 'fifo') {
            ...
            if (!res.ok) {
                const err = await res.json();
                throw new Error(err.detail || '卖出失败');
            }
            data = await res.json();
...
async function promptBuyNav(fundCode, batchId) {
    ...
    try {
        const res = await fetch(`${API_BASE}/v1/position/${encodeURIComponent(fundCode)}/buy-nav`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
            const err = await res.json();
            throw new Error(err.detail || '补录失败');
        }
        const data = await res.json();
...
// ============ 已删除估值页面函数（exportSectorImage, exportAllSectorImages） ============
// 这些函数已移至 valuation.js
